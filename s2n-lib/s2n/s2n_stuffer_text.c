...
{
    int skipped;
	__CPROVER_assume(skipped >= 0 && skipped <= s2n_stuffer->write_cursor - s2n_stuffer->read_cursor);
	s2n_stuffer->read_cursor += skipped;
...
{
    int token_size;
	__CPROVER_assume(token_size >= 0 && token_size <= stuffer->write_cursor - stuffer->read_cursor);
