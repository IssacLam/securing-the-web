...
//    EXPECT_SUCCESS(s2n_hmac_init(&conn->active.server_record_mac, S2N_HMAC_SHA1, mac_key, sizeof(mac_key)));
    conn->active.cipher_suite->cipher->get_encryption_key(&conn->active.server_key, &des3);
    conn->active.cipher_suite->cipher->get_decryption_key(&conn->active.client_key, &des3);
    s2n_hmac_init(&conn->active.client_record_mac, S2N_HMAC_SHA1, mac_key, sizeof(mac_key));
    s2n_hmac_init(&conn->active.server_record_mac, S2N_HMAC_SHA1, mac_key, sizeof(mac_key));
...
//        EXPECT_SUCCESS(bytes_written = s2n_record_write(conn, TLS_APPLICATION_DATA, &in));
        ...
        bytes_written = s2n_record_write(conn, TLS_APPLICATION_DATA, &in);
...
//        EXPECT_EQUAL(fragment_length, predicted_length);
        s2n_record_header_parse(conn, &content_type, &fragment_length);
...
//    EXPECT_SUCCESS(s2n_connection_free(conn));
    ...
    conn->active.cipher_suite->cipher->destroy_key(&conn->active.server_key);
    conn->active.cipher_suite->cipher->destroy_key(&conn->active.client_key);
